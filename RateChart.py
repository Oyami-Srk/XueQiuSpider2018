# -*- coding: utf-8 -*-
#! python3
#Author: OyaKti Studio
#Date: 2017-04-19
#This Script needs 'httplib2\pandas\PyTables' installed by pip3
...
baseHeader = {
    'Host': 'xueqiu.com',
    'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.12; rv:52.0) Gecko/20100101 Firefox/52.0',     # Programmer's User Agent
    'Accept': '*/*',
...
#祖传request, 从爬H图就开始用, 经得起时间的校验, 从py2升级而来
def request(url, body = '', header = '', method = 'GET'):
    try:
        resp, content = httplib2.Http(timeout=20).request(url, method = method, headers = header, body = urllib.parse.urlencode(body)) 
        return resp, content
    except KeyError:
        raise("Request Failed!")

#祖传获取请求头, 从爬H图就开始用, 经得起时间的校验, 从py2升级而来
def GetHeader():
    global baseHeader
    global Cookie_glo
    Header = baseHeader
...
def __GetRateChart__(Symbol):
    ...
    Header = GetHeader()
    try:
        resp, cont = request(postUrl + "cube_symbol=" + Symbol + "&until=" + str(int(time.time()) * 1000), header=Header)
    except:
...
def __SaveAsDataframe__(Data, noPercent = True):
    ...
    if not noPercent:
        dataframe = pd.DataFrame({'Value': ValueList, 'Percent': PercentList}, index=DateList)
    else:
        dataframe = pd.DataFrame({'Value': ValueList}, index=DateList)
    return dataframe
...
def GetMarketList(market = 'cn',
                  ...
                  sale_flag = '0',
                  stock_positions = '0'):
    baseUrl = 'https://xueqiu.com/cubes/discover/rank/cube/list.json?market=%s&sale_flag=%s&stock_positions=%s&sort=%s&category=12&profit=%s'
...
#返回通过All.json得到的一些信息
def __GetMarketListInfoFromMarketListsOriginDataWrittenInJsonAndReturnADataframeIndexedBySymbolWithSixColumns__(market = 'cn',
                  profit = 'monthly_gain',
                  sort = 'best_benefit',
                  sale_flag = '0',
                  stock_positions = '0'):
...
#Get TotlCount
    ...
    return pd.DataFrame({'daily_gain': DailyGain_list,
...
def GetMarketListInfo(market = 'cn',
                      ...
                      sale_flag = '0',
                      stock_positions = '0'):
    oridata = __GetMarketListInfoFromMarketListsOriginDataWrittenInJsonAndReturnADataframeIndexedBySymbolWithSixColumns__(market, profit, sort, sale_flag, stock_positions)
...
def GetPortfolioInfo(Symbol):
    ...
    day = float(data[0][0].split('>')[4].split('%')[0]) / 100
    month = float(data[1][0].split('>')[4].split('%')[0]) / 100
    net =  float(data[2][0].split('>')[4].split('<')[0])
    total = float(net - 1)
