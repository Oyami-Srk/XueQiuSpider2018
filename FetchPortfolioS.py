...
def xueqiu_login(telephone, password):
    ...
    headers = GetHeader()
    WAT.Get("https://xueqiu.com/account/login")
    time.sleep(3)
    WAT.get_element("id", "mod-login-toggle").click()
    WAT.get_element("name", "username").send_keys(telephone)
    WAT.get_element("name", "password").send_keys(password)
    WAT.select_element("value", WAT.get_element("name", "areacode"), login_areacode)
    WAT.get_element("class_name", "button").click()
    time.sleep(3)
    #WAT.__session__.save_screenshot("login.png")
    r, cont = WAT.Get("https://xueqiu.com")
    res = re.findall(r'<a href="/(\d*)/profile" data-name="(.+?)">.+?</a>', cont)
    if res == []:
        print('登录失败，请检查你的手机号和密码输入是否正确')
        return ''
    else:
        print('登录成功，你的用户 id 是：%s, 你的用户名是：%s' % (res[1]))
        return ''
...
def CheckLogin():
    return Cookie_glo_login != ''

def SetCookie(cookie):
    global Cookie_glo_login
    Cookie_glo_login = cookie

def GetHeader(login = False):
    global baseHeader
    global Cookie_glo
    Header = baseHeader
    if login:
        ...
        Header['Cookie'] = Cookie_glo_login
    else:
        if Cookie_glo == '':
            try:
...
def __GetRateChart__(Symbol):
    ...
    try:
        ...
        cont = cont.replace('</pre></body></html>','').replace('<html><head></head><body><pre style="word-wrap: break-word; white-space: pre-wrap;">','')
    except KeyboardInterrupt:
        raise KeyboardInterrupt()
    except:
